...
trait Sentry[F[_]]:
  ...
  def unsafeCapture(ev: SentryEvent): Unit
...
object Sentry:
  ...
  def stdout[F[_]: Console]: Sentry[F] =
    new Sentry[F] {
      ...
      def unsafeCapture(ev: SentryEvent): Unit = println(ev.toString)
    }
  ...
  final private class Impl[F[_]: Sync] extends Sentry[F] {
    ...
    def unsafeCapture(ev: SentryEvent): Unit =
      Sync[SyncIO].blocking(JSentry.captureEvent(ev.toEvent)).void.unsafeRunSync()
  }
